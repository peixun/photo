...
<div class="container">
	<div class="info_lf">
		...
		<div class="info_c">
            ...
            <p class="clk"><a href="__APP__/Company/reservation">预约工地</a></p> <p><a href="__APP__/Company/book">预约管理</a></p><p><a href="__APP__/Company/watchlist">收藏管理</a></p>  
	...
	<div class="info_rt">
		...
		<div class="info_msg"><span class="i_msg2" style="float:right; width:100px;"> </span><span class="i_msg1"><b>预约在线工地列表</b></span></div>
		<div class="info_cont" style="min-height:300px;">
			<table border="0" cellpadding="0" cellspacing="0" class="info_tab" width="100%">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><tr>
					...
					<td class="" style="text-align:left;">&nbsp;&nbsp;<?php echo (getCconstructionName($vo["construction_id"])); ?></td>
					<td class="" >&nbsp;&nbsp;<?php echo (getRUserName($vo["uid"])); ?></td>
					<td class=""><?php echo (toDate($vo["create_time"],"Y-m-d")); ?> </td>			
					<td class="">
                    	<p>email:<?php echo (getUserName($vo["uid"])); ?></p>
                        <p>联系方式:<?php echo (getMobile($vo["uid"])); ?></p>
                    </td>
...
function showUserinfo(id,c_id){{
	$.post('__APP__/Tool/getUserInfo',{id:id},function(res){
		if(res){
			$('#showinfo_'+c_id).html(res).show();	
		}else{
			alert("获取用户数据失败!");
		}
	});
