...
class TencentpayPayment extends Think implements Payment  {
	...
	{   
		...
		$data_return_url = 'http://tdf.265nt.cn/';
		...
		$spbil_create_ip = 	$_SERVER['REMOTE_ADDR'];
...
//		}
       ...
       // $total_fee = 0.01;
        ...
        /* 重写自定义签名 */
        //$payment['	'] = abs(crc32($payment['magic_string']));


//https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi?cmdno=1&date=20080630&bargainor_id=1202677001&transaction_id=1202677001200806300630144454&sp_billno=20080630144454&total_fee=100&fee_type=1&return_url=www.kuaer.cn/tenpay/tenpay_notify.asp&attach=tencent_davidz&sign=1FCF0CF01AE87C181DC3C286137FA2E9&bank_type=0&purchaser_id=&desc=1
        /* 数字签名 */
        $sign_text = "cmdno=" . $cmd_no . "&date=" . $today . "&bargainor_id=" . $payment_info['config']['tencentpay_id'] .
          ...
          "&attach=" . $attach ."&spbil_create_ip=".$spbil_create_ip. "&key=" . $payment_info['config']['tencentpay_key'].'&cs=gb2312';
...
///            'total_fee'         => 0.01,                  // 订单金额
			...
			'spbill_create_ip'	=>$spbil_create_ip,	
            'sign'              => $sign,                       // MD5签名
			//'cs'				=> "gb2312"
