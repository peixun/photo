...
class UserInfoAction extends CommonAction {
	...
	public function edit() {
		...
		$vo = $model->getById ( $id );
	    $industr = explode(",",$vo["industry"]);
	    $this->assign("val",$industr);
          
		$this->assign ( 'vo', $vo );
		
		$CourseCate = D ( "CourseCate" );
		$where ["pid"] = 1;
		$courseCate = $CourseCate->where ( $where )->select ();
		if ($courseCate) {
	...
	public function download() {
		...
		$gbPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( 'A1', '会员帐号' )->setCellValue ( 'B1', '电子邮件' )->setCellValue ( 'C1', '口令' )->setCellValue ( 'D1', '手机' );
	...
	function expusercsv($page = 1, $condition = '') {
		...
		$sql = "select u.id, u.user_name, u.email, u.mobile_phone, c.name as city_name, u.address from " . C ( "DB_PREFIX" ) . "user as u " . "left outer join " . C ( "DB_PREFIX" ) . "group_city c on c.id = u.city_id where 1=1 ";
	...
	function expuserxls() {
		...
		$sql = "select u.user_name, u.email, u.mobile_phone, c.name as city_name, u.address from " . C ( "DB_PREFIX" ) . "user as u " . "left outer join " . C ( "DB_PREFIX" ) . "group_city c on c.id = u.city_id";
		$list = D ()->query ( $sql );
		
		Vendor ( 'PHPExcel' );
		Vendor ( 'PHPExcel.IOFactory' );
		$userPHPExcel = new PHPExcel ();
		$userPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( 'A1', '用户名' )->setCellValue ( 'B1', '邮箱' )->setCellValue ( 'C1', '手机' );
		
		$i = 2;
		foreach ( $list as $user ) {
			$userPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( "A$i", $user ['user_name'] )->setCellValue ( "B$i", $user ['email'] )->setCellValue ( "C$i", $user ['mobile_phone'] );
			$i ++;
		}
	...
	function showdetail() {
		...
		//会员订单
		$sql = 'SELECT a.*, og.data_name, og.rec_id, u.user_name,' . ' (select sum(og1.number) from ' . C ( "DB_PREFIX" ) . 'order_goods og1 where og1.order_id = a.id) as goods_num, ' . ' (select gc.name from ' . C ( "DB_PREFIX" ) . 'group_city gc where gc.id = g.city_id) as city_name, ' . ' (a.cost_total_price + a.cost_delivery_fee + a.cost_protect_fee + a.cost_payment_fee + a.cost_other_fee) as order_cost' . '  FROM ' . C ( "DB_PREFIX" ) . 'order a left join ' . C ( "DB_PREFIX" ) . 'user u on u.id = a.user_id ' . '  left join ' . C ( "DB_PREFIX" ) . 'order_goods og on og.order_id = a.id ' . '  left join ' . C ( "DB_PREFIX" ) . 'goods g on g.id = og.rec_id where a.user_id =' . $user_id;
		$list = D ()->query ( $sql );
		$this->assign ( 'order_list', $list );
		
		//收款单
		$sql = 'SELECT a.*,' . '       b.sn as order_sn,' . '       u.user_name as user_name,' . '       b.order_total_price as  final_amount,' . '       c.name_' . DEFAULT_LANG_ID . ' as PName,' . '       d.name_' . DEFAULT_LANG_ID . ' AS AName,' . '       og.data_name,' . '       og.number' . '  FROM ' . C ( "DB_PREFIX" ) . 'order_incharge a' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'order b ON b.id = a.order_id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'payment c ON c.id = a.payment_id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'currency d ON d.id = a.currency_id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'user u ON u.id = b.user_id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'order_goods og ON og.order_id = a.order_id' . '  where b.id is not null and b.user_id = ' . $user_id . ' ORDER BY a.create_time desc';
		$list = D ()->query ( $sql );
		$this->assign ( 'order_incharge_list', $list );
		
		//发货单
		$sql = 'SELECT a.*,' . '       b.sn as order_sn,' . '       b.order_total_price as final_amount,' . '       c.name_' . DEFAULT_LANG_ID . ' as fname,' . '       d.user_name as mname,' . '       ocg.number,' . '		(select data_name from ' . C ( "DB_PREFIX" ) . 'order_goods og where og.id = ocg.order_goods_id) as data_name' . '  FROM ' . C ( "DB_PREFIX" ) . 'order_consignment a' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'order b ON a.order_id = b.id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'delivery c ON a.delivery_id = c.id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'user d ON b.user_id = d.id' . '  LEFT OUTER JOIN ' . C ( "DB_PREFIX" ) . 'order_consignment_goods ocg ON ocg.order_consignment_id = a.id' . '  where b.id is not null and b.user_id = ' . $user_id . ' ORDER BY b.sn, a.id';
		$list = D ()->query ( $sql );
		...
		//代金券信息
		$sql = "select e.id,e.sn,e.password,e.use_date_time,uu.user_name as use_user_name,et.name,et.use_start_date,et.use_end_date," . " g.name_1 as goods_name,et.money,et.status from " . C ( "DB_PREFIX" ) . "ecv as e left join " . C ( "DB_PREFIX" ) . "ecv_type  as et " . " on et.id = e.ecv_type left join " . C ( "DB_PREFIX" ) . "user as uu on uu.id = e.use_user_id left join " . C ( "DB_PREFIX" ) . "goods as g on g.id = e.goods_id " . " where e.user_id = " . $user_id . " group by e.id order by e.id desc";
