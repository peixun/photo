...
<input type="text" name="name" title="<?php echo (L("ROLE_NAME")); ?>" class="medium" >
...
<div class="list" >
<!-- Think 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr><td height="5" colspan="4" class="topTd" ></td></tr><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="8%"><a href="javascript:sortBy('id','<?php echo ($sort); ?>','index')" title="按照<?php echo (L("ID")); ?><?php echo ($sortType); ?> "><?php echo (L("ID")); ?><?php if(($order)  ==  "id"): ?><img src="__TMPL__ThemeFiles/Images/<?php echo ($sortImg); ?>.gif" width="12" height="17" border="0" align="absmiddle"><?php endif; ?></a></th><th><a href="javascript:sortBy('name','<?php echo ($sort); ?>','index')" title="按照<?php echo (L("ROLE_NAME")); ?><?php echo ($sortType); ?> "><?php echo (L("ROLE_NAME")); ?><?php if(($order)  ==  "name"): ?><img src="__TMPL__ThemeFiles/Images/<?php echo ($sortImg); ?>.gif" width="12" height="17" border="0" align="absmiddle"><?php endif; ?></a></th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$role): ++$i;$mod = ($i % 2 )?><tr class="row" onmouseover="over(event)" onmouseout="out(event)" onclick="change(event)" ><td><?php if(($role['level'] == '0')or($role['level'] > 0)): ?><?php else: ?><input type="checkbox" class="key" name="key"	value="<?php echo ($role["id"]); ?>" <?php if($role['checked']): ?>checked="checked"<?php endif; ?> ><?php endif; ?></td><td><?php echo ($role["id"]); ?></td><td><a href="javascript:edit('<?php echo (addslashes($role["id"])); ?>')"><?php echo ($role["name"]); ?></a></td><td> <?php echo (showStatus($role["status"],$role['id'])); ?>&nbsp;<a href="javascript:edit('<?php echo ($role["id"]); ?>')"><?php echo (L("_EDIT_DATA")); ?></a>&nbsp;<a href="javascript:foreverdel('<?php echo ($role["id"]); ?>')"><?php echo (L("_DELETE_DATA")); ?></a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?><tr><td height="5" colspan="4" class="bottomTd"></td></tr></table>
<!-- Think 系统列表组件结束 -->
